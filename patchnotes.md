...
worker's multi-statement sets run in transactions, the medium bug
cluster closes, and the dependency platform moves to rusqlite 0.40 +
gtk4 0.11 ahead of the 1.0 freeze (decision 61).**
...
- **Changed:** the dependency platform moves before the 1.0 freeze (decision 61, Brandon 2026-09-13): `rusqlite` 0.32 to 0.40 and `gtk4` 0.9 to 0.11 (gtk-rs-core follows; glib/gio land on 0.22). No new crates. The gtk4 bump required a matching `vir-gtk` 1.3.0 first, because gtk4 0.9 and 0.11 are distinct crates whose widget types cannot cross the boundary; that release ports the portal's `SettingChanged` listener to gio 0.22's `subscribe_to_signal` and waives Conservatory/Viaduct to their next releases (no API they call changed; the waiver is recorded in vir-gtk's roadmap). The lock pins vir-gtk `ae08486` and `data/cargo-sources.json` is regenerated against it. Compile fallout was small: gtk4 0.11 requires subclasses to list every implemented interface in `glib::wrapper!` (AtriumWindow, AtriumClamp), implementing Root made `focus()` resolution ambiguous in the F2 edit path (disambiguated to `GtkWindowExt::focus`, the same GTK call as before), and the spec §3.4 SQL-trace hook ports from rusqlite's deprecated `profile` to `trace_v2` (the log line keeps the SQL text via `StmtRef::sql`).
- **Changed:** CI gains a `cargo-sources.json` freshness guard: the fedora job regenerates the manifest from `Cargo.lock` and fails on drift, closing the recurring stale-manifest class that bit v0.70.0 and v0.72.1.
- **Fixed:** the reminder service never fired. The loop was spawned on the GLib main context but slept with `tokio::time::sleep`, which panics outside a tokio runtime; the main context swallowed the unwind and the reminder future died on its first timer branch. The loop now sleeps on `glib::timeout_future`; the `WorkerHandle` awaits stay (mpsc and oneshot need no reactor). Read failures are no longer mistaken for "nothing pending": the pending query, the fire-time task read, and a failed fire recording all warn and back off 60 s. An end-to-end regression test drives the real loop (real pool, worker, GLib timers, private main context, memory GSettings backend) against a temp-file DB with a reminder ~1.5 s out so the sleep branch is the code under test; a spy replaces the gio notification, and the reminder must fire exactly once, be recorded in `task_reminder_fired`, and not repeat.
- **Fixed:** four multi-statement worker sets committed without a transaction, so a mid-set failure persisted partial state: `clock_in` (auto-close + insert), the repeat follow-up spawn (INSERT + tag copy inside `toggle_complete`), `create_task_template` (template + items), and `instantiate_template` (project + tags + items + tag attachment). `toggle_complete` also honors its documented failure policy now: a failed follow-up spawn logs a warning and returns no follow-up over the committed completion instead of propagating an error the caller reads as a failed toggle. Connection-parameterized inner helpers keep the sites nest-free.
- **Fixed:** a kanban Tag-axis drop wiped the dragged card's tags when the tag-pool read failed (the failure read as an empty current set, and the move committed just the destination column's tag); the drop now skips the membership change on a failed read while the ordering write still applies. Vault conflict backups no longer collide at second resolution (`~N` suffix ported from the DB-snapshot backup scheme; the older loser was the only copy of an external edit). `seed_fresh_vault` runs under `spawn_blocking` and reads through the read-only pool instead of opening a second writable connection for read-only work.
- **Docs:** the six-lens audit's docs sweep: spec §3.3 describes the six-crate workspace plus the vir-search / vir-gtk git dependencies (the dead `atrium-search` naming is gone from spec §4.4/§4.5's execution paths too); `docs/org-roundtrip.md` no longer presents the v0.24.0-fixed custom-property drop as a current limitation; the three "Phase 24" pointers cite v0.72.0 where that work is actually recorded; the keymap's stub table is honest and the three bound-but-undocumented accels (`Ctrl+F`, `Ctrl+Alt+Space`, `Ctrl+Shift+T`) are documented; spec §5.1/§5.2's widget trees show the shipped plain-GTK `GtkPaned` layout; `docs/perf-baseline.md` is genuinely re-baselined on the new dependency stack (50K load 300 ms / 57 MB peak, 100K 590 ms / 104 MB, cold-start floor 30 ms; PASS); CLAUDE.md's codebase map loses its phantom modules and gains the real ones.
...
The runtime side is a new `atrium/src/i18n.rs`: the `atrium` text domain is bound at startup (before any GTK type exists, so GtkBuilder templates translate at inflate time), with the localedir resolved runtime `ATRIUM_LOCALEDIR` → compile-time default baked by `build.rs` → `/usr/share/locale`. The module re-exports `gettext` / `ngettext` / `pgettext` and adds two named-placeholder helpers (`gettext_f`, `ngettext_f`) because translated strings can't pass through `format!`; translators can reorder `{name}` placeholders freely. The dependency is `gettext-rs` with the `gettext-system` feature (links glibc's built-in gettext; nothing vendored) — sign-off was pre-granted by the roadmap item naming it.

The build side is the standard GNOME shape: `po/` with `POTFILES`, `LINGUAS`, a checked-in `atrium.pot`, and `en.po`; meson gains the `i18n` module, `subdir('po')` (glib preset + the helper keywords), and `i18n.merge_file` for the desktop entry and metainfo, which are now `.in` sources. The GSettings schema carries `gettext-domain="atrium"`, `window.ui` carries `domain="atrium"` (its 28 `translatable="yes"` marks finally do something), and the keyboard-shortcuts XML moved out of an inline Rust const into `data/shortcuts.ui` (still `include_str!`-compiled, no install change) so xgettext can extract its titles. Regenerate the pot with `meson compile atrium-pot` (xgettext ≥ 0.25 for the Rust extraction).

The sweep marked every user-facing string in the GTK binary — labels, titles, toasts, tooltips, placeholders, dialogs, accessible descriptions — while `atrium-cli`, the library crates, the `--debug` tooling, and log output stay untranslated on purpose (script-parseable contract surface and diagnostics, not prose). File-format and search-grammar tokens (`TODO`, `is:overdue`, `#tag`) stay literal inside otherwise-translatable sentences, pinned by translator comments. `DateBand::heading()` and the calendar's `month_name()` now return translated `String`s.

Two conventions start here: every future metainfo `<release><description>` carries `translate="no"` (the 142 existing entries were marked; release notes don't belong in the translation catalogue), and new `.rs` files with user-facing strings must join `po/POTFILES`. One long-standing drift also closes: meson's `project()` version now reads the `VERSION` file instead of a hardcoded string that had been stuck at 0.5.0.

No schema change. No behaviour change in the C locale: with no catalogue for the current language, every `gettext` call returns its English msgid unchanged, which is also why the existing English-string test assertions keep passing (tests never bind the domain). Spec gains §3.6 documenting all of this.

By the numbers: the pot carries 536 msgids from 32 sources (27 Rust files, the two `.ui` files, gschema, desktop entry, metainfo), with 12 plural pairs and 92 translator comments. Workspace headline count rises 1,023 → 1,027 with the i18n helper's unit tests.

Verification rode along and caught two things worth noting. First, the DESTDIR-staged `meson install` exposed a day-one bug: the `glib-compile-schemas` install script ran against the absolute `$datadir` path, so any staged install failed (it only ever worked because Flatpak installs without DESTDIR); it is now `gnome.post_install(glib_compile_schemas: true)`, which skips itself under DESTDIR the way packaging expects. Second, the current clippy (rust 1.97.1) newly flags two pre-existing patterns — `useless_borrows_in_formatting` in atrium-core's read module and `question_mark` in the Org repeater parser and the Todoist ordinal-day parser — all three fixed mechanically to keep the `-D warnings` gate green. Full verification: `scripts/regression.sh` PASS, `meson install` into a DESTDIR stage with `atrium.mo` landing in `share/locale/en/LC_MESSAGES/`, `desktop-file-validate` and `appstreamcli validate` clean on the merged outputs, and pot regeneration stable to the creation date.
