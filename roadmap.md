...
- [ ] **Performance regression suite** *(v0.36.0):* scripted load against the 50K / 100K fixtures asserting the §8 budgets; `docs/perf-baseline.md` refresh; opt-in heaptrack arm.
- [ ] **Localisation scaffolding** *(v0.37.0):* `gettext-rs`, `po/` + meson i18n, marked Rust strings, `atrium.pot`, ship `en`. Translation-ready; real locales come later from contributors (scaffolding-only per the Phase 20 decision — no pilot translations committed at 1.0).
- [ ] **Documentation site** *(v0.38.0):* `mdbook` assembled from `docs/` + spec/README; user manual, keymap, import guide, schema reference, RRULE subset.
- [ ] **Flathub readiness** *(v0.39.0):* offline cargo-sources (drop the network share), `<screenshots>` metainfo scaffold. Screenshot capture + the actual Flathub PR are Brandon's (display + account).
- [ ] **AppStream screenshots refresh** — Simple and Builder both featured (Brandon captures; the XML scaffold lands in v0.39.0).
- [ ] **Final icon and brand pass.**
- [ ] **`v1.0.0` tag, release notes, retrospective** — major-bump maintenance pass + the annotated tag.
