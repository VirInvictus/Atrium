...
- [x] **C1 — Foundations + priority fix** *(shipped v0.50.0).* About dialog → `gtk::AboutDialog` (adwaita's acknowledgement/legal sections map onto `add_credit_section`; `issue_url` has no analogue so the tracker rides the website label). CSS-provider-priority bug fixed: the sheet moved from `PRIORITY_APPLICATION` to `STYLE_PROVIDER_PRIORITY_USER + 1` (`atrium/src/ui/typography.rs`), so a system-wide Kanagawa `~/.config/gtk-4.0/gtk.css` no longer half-overrides Atrium (the Colophon/Conservatory fix). The keyboard-shortcuts window is **deferred, not done**: it is already plain `gtk::ShortcutsWindow` (no adwaita to remove), and the `GtkShortcuts*` deprecation only bites at the `v4_18` gate while Atrium pins `v4_16`, so converting it now — before the C5 owned-rows module exists — would be throwaway. It rides C5, or a later `v4_18` bump. No visual change.
