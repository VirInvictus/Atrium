...
- [x] **Review perspective:** new `read::list_review_queue(conn, today)` SQL selects projects with `review_interval_days IS NOT NULL`, `archived_at IS NULL`, and `last_reviewed_at + review_interval_days days ≤ today` (or never reviewed). Order: never-reviewed first, then by oldest `last_reviewed_at`, then by `position`. The Phase 10 Review sidebar stub now mounts a real `atrium/src/ui/review.rs` page that renders one card per queued project (title, area subtitle, "Last reviewed N days ago" / "Never reviewed" caption, Mark Reviewed button). Empty queue shows an `AdwStatusPage` "All caught up" placeholder. **v0.7.2 follow-up:** the Review page now renders TWO sections — "Projects to review" (this Phase 13 queue) followed by "This week" (open tasks matching `is:overdue OR scheduled:thisweek OR (is:deadline AND due:nextweek) OR (is:deferred AND defer:<=today)`, the filter formerly seeded as the standalone "Weekly Review" Perspective). The seed retired alongside the merge — the canonical Review surface is now the single home for both. Status placeholder shows only when both sections are empty.
- [x] **"Mark Reviewed" action** — `Command::MarkReviewed { id }` worker variant + `WorkerHandle::mark_reviewed(id)` API. Handler runs `UPDATE project SET last_reviewed_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?1`, emits `LibraryChanges{projects_updated}`. The card's Mark Reviewed button dispatches via `glib::MainContext::default().spawn_local`, disables itself while in flight to prevent double-fire, and drops out of the visible queue when `apply_library_changes` triggers a page rebuild. **v0.7.4 follow-up:** the same pattern lands at the task level. Migration `0006_task_last_reviewed_at.sql` adds `task.last_reviewed_at`; `Command::MarkTaskReviewed { id }` + `WorkerHandle::mark_task_reviewed(id)` mirror the project-side wiring, emitting `TaskChanges{updated}`. The Review page's "This week" weekly walk gains a per-row Mark Reviewed button; the page filter excludes tasks reviewed within the last 7 days.
...
## Phase 15.75: Atrium polish, search-engine evolution, atrium-cli, Slice D
*Visual polish + per-area accent + Phase 15.5 deferred-list closure + the search engine and a full headless CLI extracted as their own workspace crates + the Slice D renderer system (kanban + agenda).* Slices A + B + the v0.4.x patch sequence shipped at **v0.5.0**; Slices C + D + the v0.6.x carryover items shipped across **v0.5.1 → v0.6.7**.
...
- [x] **atrium-cli workspace crate (v0.4.3 → v0.4.7, extended v0.5.x → v0.6.x).** Headless binary with full task CRUD plus metadata reads. Reads (`search`, `list`, `info`, `kanban`) open `SQLITE_OPEN_READ_ONLY`; writes (`add`, `capture`, `edit`, `complete`, `delete`, `perspective`) spin up the worker on a current-thread tokio runtime. TSV (default) / JSON / human output formats. Quick Entry parser (`#tag` / `@today` / `@deadline ...`) lifted from the GTK binary into `atrium_core::quick_entry` so atrium-cli's `capture` reuses the exact same grammar.
- [x] **CLI bulk operations (v0.4.6).** `atrium-cli complete --where 'is:overdue'` and `atrium-cli delete --where 'is:done'` walk every match. Delete defaults to dry-run (exits status 2) so a caller can review the proposed rows before re-running with `--force`.
- [x] **Regression-script integration (v0.5.x).** `scripts/regression.sh` step 5.5 exercises atrium-cli end-to-end against the `--fixture small` DB — read paths, search operator classes, JSON formatter, full add → info → search → edit → complete → delete write round-trip, bulk dry-run / force, kanban render, perspective create/edit/delete round-trip. Closes the architectural commitment that every non-GUI surface stays CLI-testable.
- [x] **Slice C — GTD audit fixes (v0.5.0 → v0.6.0).** First-run seed of a Weekly Review Perspective (`atrium_core::WEEKLY_REVIEW_NAME` + `WEEKLY_REVIEW_FILTER`). Logbook day-grouping headers (Today / Yesterday / Last 7 Days / Older) via `atrium/src/ui/logbook.rs`. `docs/gtd-patterns.md` documents the `#waiting`, `#delegated`, `#blocked` tag idioms.
- [x] **Slice D — kanban board renderer + Agenda canonical page (v0.5.4 → v0.6.5).** Two subslices, both shipped:
  - **D1: Kanban board (v0.5.4 → v0.6.6).** `atrium-core::render` module hosts the typed `Renderer { List | Board(BoardConfig) }` enum, `BoardConfig::{to_json,from_json}`, `group_into_board` grouper, and `move_to_column` tag-rewrite helper (eight unit tests pin the leftmost-match-wins / "Other" trailing column / case-insensitive rules). Saved Perspectives whose `renderer = "board"` render as a horizontal column layout with one column per configured tag. Drag-drop between columns rewrites the task's tag set via `move_to_column` + `worker.set_task_tags`. Interactive completion checkbox; click rows open in Inspector. `atrium-cli kanban NAME` renders the same engine to the shell. `Configure renderer…` dialog on the Perspective row's right-click menu lets users convert list ↔ board and edit columns. `atrium-cli perspective <create|edit|delete>` provides the matching shell write side. v0.6.6 added the SQL translation evaluator's fast-path on the GUI's per-refresh load and dropped the hover-transition CSS that was producing CPU spikes during drag motion.
  - **D2: Agenda canonical page (v0.6.4).** Org-mode-style "everything you should think about right now" view. New `ActiveList::Agenda`; sidebar entry alongside Forecast / Review (top-tier in v0.6.7, mode-agnostic). Five chronological sections — Overdue / Today / Tomorrow / This Week / Next Week — with the Overdue heading rendered in `@warning_color` so urgency reads at a glance. `atrium/src/ui/agenda.rs` hosts the `classify` / `group_by_section` / `build_page` trio (fourteen unit tests pin the rules: completed-skip, deferred-future-skip, no-anchor-skip, someday-skip, overdue-precedence, most-imminent-wins, this-week / next-week boundaries, beyond-skip).
- [x] **FTS5 bm25 + recency ranking (v0.5.2).** Bare-text searches now rank by FTS5 `bm25` blended with a 30-day half-life recency factor. `atrium-search::collect_text_terms` + `blend_relevance` are pure helpers (twelve unit tests on the math). `atrium-core::db::read::bm25_for_terms` queries the FTS5 virtual table; `atrium/ui/filter::rank_by_bm25_recency` wires it into the GUI search-bar and saved Perspective paths. `atrium-cli` `run_search` does the same on the shell side.
- [x] **SQL-translation evaluator (v0.5.3).** `atrium-search::sql_translate::try_translate(&Expr, today)` converts a parsed expression to a SQL `WHERE` fragment + bound params when every node maps cleanly. Returns `None` for regex / fuzzy / `is:today` / `Field::Project|Area` (deferred) so the in-memory evaluator stays the fallback. `atrium-core::SqlBindValue` is the wire-level binding type so binaries can compose without a direct rusqlite dep. 21 parity integration tests pin the SQL ↔ in-memory agreement on a mixed-shape fixture. atrium-cli `filtered_tasks` and the GUI's `refresh_board_page` both use the fast path.
- [x] **Sidebar reorganisation (v0.6.7).** Agenda / Forecast / Review moved out of the bottom "Builder" section header and up into the top tier alongside Inbox / Today / etc., with their own accent tints. Agenda is mode-agnostic; Forecast and Review join the top tier in Builder mode only. Perspectives section moves up to between the top-tier group and Areas.
