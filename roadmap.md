...
What's done, what's next, what's deferred. Sequenced for a clean Simple Mode v0.1, a Builder Mode v0.2 expansion, and a 1.0 with broad import/export across the Linux task-app ecosystem. Current release: **v0.10.2** (Phase 17 reliability slice — malformed-file pause/resume; custom-keyword preservation through the watcher path; file-removal toast that retains tasks per spec §3.5; three of four §17 test scenarios pinned). v0.10.3 (RRULE canonicalisation on emit, RRULE divergence detection on read-back, agenda-parity acceptance test) closes Phase 17. Phase 12.5 (Calendar Month View — currently subsumed by Agenda) needs a re-engagement decision.
...
- [~] **Test scenarios** (partial — v0.10.2): synthesized concurrent edit (✓ `concurrent_atrium_and_external_edit_preserves_user_content_as_bak`), malformed-file recovery (✓ `malformed_file_pauses_then_recovers`), large-file 1K-task parse latency (✓ `large_file_parses_under_budget`). Multi-day RRULE round-trip preserving `:RRULE:` lands with v0.10.3's RRULE work.
