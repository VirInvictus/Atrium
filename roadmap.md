...
What's done, what's next, what's deferred. Sequenced for a clean Simple Mode v0.1, a Builder Mode v0.2 expansion, and a 1.0 with broad import/export across the Linux task-app ecosystem. Current release: **v0.10.1** (Phase 17 next slice — GUI participates in the two-way loop, conflict detection preserves external edits, sidecar config round-trips tag colours; cleanup pass wires DomainError + UiError + AtriumError properly and fixes the v0.10.0 `flatten_one` regression). v0.10.2 (malformed-file pause/resume) and v0.10.3 (RRULE divergence + agenda parity) close the Phase 17 patch arc.
