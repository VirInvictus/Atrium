...
- [ ] **Custom TODO sequences (Org `#+TODO: TODO NEXT WAITING | DONE CANCELLED`).** Across every "how I org" post — Bernt Hansen's NEXT-replaces-priority workflow, Jethro Kuan's processing pipeline, every GTD-with-Org tutorial — the user's own keyword set is the *first* thing configured. Doom exposes it as a default config point because everyone overrides it. Atrium already stores arbitrary keywords via `task.orig_keyword` (migration 0007) — what's missing is **per-vault declared sequences** so the writer emits `#+TODO:` and the watcher validates incoming keywords against the configured set. Sidecar carries the sequence list (`[todo_sequences]` section in `.atrium/config.toml`); writer projects `#+TODO:` on each project file's preamble; Inspector lets the user pick from the configured set instead of typing arbitrary text. Builder-only — Simple Mode keeps the TODO/DONE binary. **Schema impact: zero** (orig_keyword already exists; sequences live in the sidecar). *Source: norang, Doom, Worg, Jethro Kuan.*

- [ ] **CLOCK time tracking (Org `org-clock-in` / `org-clock-out`) with LOGBOOK projection.** One of the top two reasons real users stay on Org instead of Things/Todoist. Distinct from `estimated_minutes` (intent) — records *actual* time spent, accumulated across multiple work sessions. New `task_clock_entry` table (`id`, `task_id`, `started_at`, `ended_at` NULL when in-progress, `note` optional). Inspector pane gains a Start/Stop button + total time + per-session log. Round-trips to Org as `CLOCK: [start]--[end] => HH:MM` lines under a `:LOGBOOK:` drawer; suppress emission when an in-progress clock isn't yet closed (avoid churn). The watcher learns to parse `CLOCK:` lines as structured rather than verbatim so external Emacs clock-in / clock-out flows back into the DB. Builder-only. *Source: norang ("clocking fanatic"), Sacha Chua's clock-tables, cmdln.org-2024.* The largest schema-affecting item in 18.5.

- [ ] **Statistics cookies on parent headlines (`[2/5]` / `[40%]`).** Karl Voit explicitly names progress cookies as one of the *very few* features he uses. Atrium has `parent_id` already; this is emit + parse work, no schema. Project rows in the sidebar and parent task titles in the list gain an inline `[<done>/<total>]` cookie; emitter appends the cookie to project headings + any nested heading with TODO children; parser strips it on read so it doesn't pollute task titles. The `read::counts` plumbing already computes the ratios. **Schema impact: zero.** Both modes — Simple Mode already shows Things-style "5 of 12" project counts in the sidebar; this is the same number, projected back into the vault. *Source: Karl Voit, every Org project-management tutorial.*

- [ ] **DEADLINE warning windows (Org `-Nd` / `--Nd`).** Per-deadline lead times — `DEADLINE: <2026-05-15 Fri -7d>` means "surface 7 days early in Forecast." Atrium currently treats every deadline uniformly; per-task overrides let users say "this one is sensitive." One column on `task` (`deadline_warn_days INTEGER NULL` — additive migration); Forecast + Today honour it. Round-trip is mechanical — parser captures the suffix, emitter writes it back. Builder-only. **The lowest-cost Tier-1 item — could land first as a warm-up.** *Source: nvim-orgmode issue tracker, orgmode.discourse.*

- [ ] **Quick Entry templates (Atrium's read of Org capture templates).** This is the *under-recognised* item — capture templates topped the Worg survey (#1 feature) and every power-user (Bernt Hansen, cmdln.org-2024 with 24 templates, Jethro Kuan, System Crafters, Doom Emacs's `+capture` integration) builds workflows around them. Atrium already ships Quick Entry (`Ctrl+Alt+Space`); the gap is **template multiplicity** — real users have 5-25 named templates routing to different files / projects with pre-filled tags + prefixes. New `quick_entry_template` table (`id`, `name`, `shortcut_key` optional, `target_project_id` nullable, `prefix` text, `default_tags` JSON array). Quick Entry modal grows a template picker: type the shortcut letter, the template's prefix + tags pre-fill, the user types their content + commits. Builder-only — Simple Mode keeps the single Quick Entry shape. *Source: Worg survey, norang, cmdln.org-2024, every "how I org" post.*

### Tier 2 — high-value-for-some, ship if scope allows (3 items)

- [ ] **Org links between tasks (`[[id:UUID][label]]`).** ID-based linking is what `org-roam` is built on; Karl Voit explicitly advocates ID links for portability. Atrium already generates `:ID:` UUIDs and they round-trip — adding a "Link to task..." picker in the Inspector body editor that inserts `[[id:UUID][title]]`, plus a body renderer that turns those links into clickable jumps to the target task, unlocks org-roam-adjacent workflows almost for free. **Schema impact: zero** (UUIDs exist, body is already verbatim). *Source: cmdln.org-2024, Karl Voit's UOMF: Linking Headings, org-roam ecosystem.*

- [ ] **Body inline checkboxes (`- [ ]` / `- [X]` with state).** Karl Voit names checkbox lists alongside statistics cookies as essential. They're how power users avoid the headline-explosion problem (one TODO with five steps, not six TODOs). Render-time work: the Inspector body parser recognises `- [ ]` / `- [X]` lines and renders interactive toggles that rewrite the body string in place. Pairs naturally with Statistics cookies — when present, the cookie should count checkboxes as well as child TODOs (Org's `org-hierarchical-todo-statistics` does this). **Schema impact: zero.** Both modes — Things 3 and OmniFocus both ship checkbox lists in notes; this is parity at minimum. *Source: Karl Voit, Jeff Bradberry's lists-and-checklists, Christian Tietze.*

- [ ] **Time-of-day on `scheduled_for`.** The Todoist importer's `DroppedTimeOfDay` lossy entry is a real signal — users schedule tasks at specific times (`SCHEDULED: <2026-05-20 Wed 14:00>`) and Atrium's date-only column drops the precision. `ScheduledFor::Date(NaiveDate)` becomes `ScheduledFor::DateTime(NaiveDateTime)` *or* a sibling variant (`ScheduledFor::DateAt(NaiveDate, NaiveTime)`); migration adds `task.scheduled_time TIME NULL`; Forecast + Calendar gain time-strip rendering for the day-view. Builder-only at first — Simple Mode's "Today" doesn't surface the time. Adjacent to (and naturally bundled with) the DEADLINE-warning-windows item since both touch scheduling. *Source: Todoist mapper's lossy report, every "Org vs Todoist" comparison thread.*
