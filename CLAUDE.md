...
**v0.6.0 carryover.** Phase 15.75 Slices C (GTD audit fixes — Weekly Review seed Perspective + Logbook day-grouping headers + `docs/gtd-patterns.md`) and D (saved Perspectives can render as kanban boards via the schema columns shipped at v0.5.0). Search-engine deferred items: SQL pushdown (perf optimization at the 100K-task scale), FTS5 bm25 + recency ranking on bare-text searches. CLI carryovers: bulk operations (`atrium-cli complete --where 'is:overdue'`) + regression-script integration that exercises `atrium-cli` end-to-end against a fixture DB.
