...
- **Conflict-detection backup format is `<file>.atrium.bak.<YYYYMMDDTHHMMSSZ>`, with a `~N` disambiguator for same-second collisions** (`~1`, `~2`, ...; v0.73.0; the older loser is the only copy of an external edit, so a collision must not overwrite it). Filesystem-safe (no colons), UTC, sortable. Don't use RFC 3339 with colons: it works on Linux ext4 but is unreliable on FAT32 / SMB shares.
...
- **A version-stamp commit replaces the `VERSION` line, never deletes it.** The v0.74.0 stamp emptied the file and wrote nothing; meson reads the file directly (`files('VERSION')`) and reads an empty file as an empty version string, so the tree silently mis-versioned instead of failing. The sync is now enforced mechanically in three places: `scripts/regression.sh` step 0 (non-empty and equal to the workspace `Cargo.toml` version, fails fastest), the CI guard step beside the cargo-sources guard, and the `version_file_is_nonempty_and_matches_workspace_version` test in `atrium-cli`.
...
```bash
cargo test --workspace            # all tests (995 at v0.74.0; the v0.70.0 vir-search extraction moved the search suite out of the workspace)
...
├── src/vault_writer.rs               ← `VaultWriter` task: debounced project flushes; pre-flush conflict check copies external edits to <file>.atrium.bak.<UTC>[~N] (same-second collisions get the ~N disambiguator, v0.73.0); refreshes sidecar via `last_sidecar` cache
