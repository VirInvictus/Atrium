...
**Current release: v0.10.1** (May 2026). Phase 16 (Org-mode import + DB → vault writer) shipped at v0.8.0; v0.9.0 lifted the Org projection into its own `atrium-org` workspace crate; **v0.10.0 shipped the Phase 17 first slice — vault → DB sync** (`notify`-backed `VaultWatcher`, `RecentWrites` self-write filter, reader→DB diff by `:ID:`); **v0.10.1 ships the next slice — GUI wiring, conflict detection, sidecar.** New `spawn_vault_loop` builder (replaces the broken `spawn_org_vault_with_watcher`); `VaultEvent` channel surfaces ConflictBackup / ParseFailed events to GUI toasts; writer snapshots external edits to `<file>.atrium.bak.<UTC-timestamp>` before overwrite (spec §7.3.3 rule 5); `<vault>/.atrium/config.toml` sidecar round-trips tag colours (hand-rolled minimal TOML — no `toml` crate dep); worker now enforces `DomainError::ParentProjectMismatch` + `EmptyFilterExpr`; `UiError::VaultPathInvalid` + `AtriumError` replace four-year-old `#![allow(dead_code)]` scaffolding; `flatten_one` bug fix (TODOs under non-keyword headings now flow through). Malformed-file pause/resume + RRULE divergence + agenda-parity acceptance test land at v0.10.2 → v0.10.3.
...
**Test count: 611 across the workspace at v0.10.1**, all green. `bash scripts/regression.sh` runs in under 2 seconds. Schema version: 7.
