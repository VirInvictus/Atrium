...
**Current release: v0.71.1** on `main` (September 2026; v0.70.0 extracted the search parser and shared GTK widgets into `vir-search` / `vir-gtk`; v0.71.0 closed the Phase 23 codebase sweep — two-way sync body/`:EFFORT:`/`:DEFER_UNTIL:`, search date-boundary parity, ensure-delta, mid-token completion — plus the Phase 21 agent-executable tail: Builder pane toggle + first-run width, sidebar-filter switcher, compact-width idiom, Hyprland docs, accessibility re-audit; v0.71.1 fixed the fresh-install serif family-name miss found by the sandbox font verification). The `phase-22-de-adwaita` branch was merged back and deleted at v0.65.1; the vault-ledger fixes that shipped on the release line as v0.48.0 / v0.48.1 are recorded under their branch numbers, v0.60.0 / v0.60.1 (both lines independently minted a v0.48.0; the branch's is the de-adwaita re-sequence docs commit). **Schema version: 20** (migrations `0001` → `0020`; 0020 is the C9 swatch recolour). Full workspace suite green. **The Phase 22 de-adwaita ladder is complete (C1 → C10): Atrium is plain GTK4 with a self-contained owned Kanagawa Dragon stylesheet, zero libadwaita in the tree.** Display-verified and look approved by Brandon.
