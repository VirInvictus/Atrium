...
**Version:** 0.10.1 (Phases 0–16 shipped; v0.9.0 extracted Org projection into its own `atrium-org` workspace crate. **Phase 17 first + next slices shipped at v0.10.0 / v0.10.1.** v0.10.0: `notify`-backed `VaultWatcher` task; `RecentWrites` self-write filter (mtime-based exact match — see *implementation note* below); reader→DB diff by `:ID:` covering CREATE / UPDATE / DELETE; `:ID:` allocation for headlines added in Emacs without one; `TaskUpdate.completed_at` setter so `CLOSED:` cookies round-trip into the DB without stamping `now()`. v0.10.1: **GUI participates in the two-way loop** via `spawn_vault_loop` (replaces the broken `spawn_org_vault_with_watcher`); **conflict detection** preserves external edits as `<file>.atrium.bak.<UTC-timestamp>` per rule 5 below; **sidecar config** at `<vault>/.atrium/config.toml` round-trips tag colours; **`VaultEvent` channel** surfaces ConflictBackup / ParseFailed as toasts; **DomainError** + `UiError` + `AtriumError` enforce subtask-stays-in-project + non-empty-filter invariants and replace dead-code scaffolding; **flatten_one bug fix** — TODOs nested under non-keyword headings now flow through the watcher path. Malformed-file pause/resume, RRULE divergence detection on read-back, and the agenda-parity acceptance test land at v0.10.2 → v0.10.3. **Implementation note** on the self-write filter: a path-only TTL filter is too coarse — external edits within the TTL window get swallowed. mtime-based exact-tuple matching (`(path, mtime_just_written)`) is the working design; ext4 stores nanosecond mtime so two distinct writes never collide.)
