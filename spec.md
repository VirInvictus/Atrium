...
**Version:** 0.10.3 (Phases 0–17 shipped; v0.9.0 extracted Org projection into its own `atrium-org` workspace crate. **Phase 17 closed at v0.10.3.** v0.10.0: `notify`-backed `VaultWatcher`; `RecentWrites` self-write filter (mtime-based exact match — see *implementation note* below); reader→DB diff by `:ID:`. v0.10.1: GUI participates in the two-way loop via `spawn_vault_loop`; conflict detection preserves external edits as `<file>.atrium.bak.<UTC-timestamp>`; sidecar at `<vault>/.atrium/config.toml`; `VaultEvent` channel surfaces toasts; `DomainError` + `UiError` + `AtriumError` enforce subtask-stays-in-project + non-empty-filter invariants. v0.10.2: malformed-file pause/resume; custom-keyword preservation through the watcher path (fixing two v0.10.0 bugs); file-removal toast that retains tasks per §3.5; concurrent-edit + 1K-task parse latency test scenarios. **v0.10.3: RRULE canonicalisation contract end-to-end** — writer emits both Org cookie + `:RRULE:` property via new `rrule_cookie` helpers (`rrule_to_org_cookie` / `rrule_to_org_repeater` / `cookie_matches_rrule`); watcher syncs `:RRULE:` to DB and detects divergence (cookie-only edits rewritten from canonical); multi-day RRULE round-trip fixture pins all three migration cases; agenda-parity acceptance test (synthesised vault + spec-derived reference classifier) closes Phase 17. v0.11 opens Phase 18 (Todoist CSV). Phase 12.5 (Calendar Month View) is re-engaged from its earlier "subsumed by Agenda" framing. **Implementation note** on the self-write filter: a path-only TTL filter is too coarse — external edits within the TTL window get swallowed. mtime-based exact-tuple matching (`(path, mtime_just_written)`) is the working design; ext4 stores nanosecond mtime so two distinct writes never collide.)
