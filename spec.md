...
- **Atrium ships the dark Kanagawa Dragon sheet.** The `theme` apply (boot + Preferences) sets GtkSettings' `gtk-application-prefer-dark-theme` in place of `adw::StyleManager`; the `theme` GSettings key stays, but with a dark-only palette "dark" and "auto" both render dark and "light" only nudges the prefer-dark hint for any GTK-default-drawn bits the owned sheet does not cover. A true light (Lotus) palette and a portal-following auto mode are post-1.0 work. **Decoration posture (decided 2026-07-17, for the C8 shell cut): window-control buttons are hidden.** On a tiling compositor the compositor owns close/maximize/minimize, so Atrium's headerbars carry only their functional controls (menu, search, new-task, per-dialog actions) and no close/max/min cluster; `Ctrl+Q` quits, and the compositor's binds close windows. This matches the portfolio pattern (Hermitage Phases 13/14, Viaduct v3.0.0). GNOME-Shell users still close through the shell's own affordances. No functionality may depend on chrome a user has hidden.
