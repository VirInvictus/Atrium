...
**Version:** 0.11.0 (Phases 0–17 shipped; Phase 12.5 shipped at v0.11.0 — the third lens over Atrium's task data: Forecast strip, Agenda chronological bands, **Calendar Month View paper-calendar grid**. Builder-only canonical page. The earlier "subsumed by Agenda" framing was wrong; the lens is genuinely different. v0.11 ships the grid + month nav (prev/next/today/picker + Page Up/Down + Ctrl+Shift+M) + drag-to-reschedule + single-click peek-popover + double-click date-scoped drill + narrow-window collapse to vertical week strip below 600 px. Phase 17 (vault → DB two-way sync) closed at v0.10.3 — `rrule_cookie` helpers, writer emits both Org cookie + `:RRULE:`, watcher syncs `:RRULE:` and detects cookie-only divergence, agenda-parity acceptance test pins semantic parity with stock org-agenda. v0.10.0 → v0.10.2 first established vault → DB sync (`notify`-backed `VaultWatcher`, `RecentWrites` mtime self-write filter — see *implementation note* below), GUI wiring (`spawn_vault_loop` + `VaultEvent` channel), conflict detection (`<file>.atrium.bak.<UTC-timestamp>`), sidecar (`<vault>/.atrium/config.toml`), real `DomainError` / `UiError` / `AtriumError`, malformed-file pause/resume, custom-keyword preservation, file-removal toast. Phase 18 (Todoist CSV) is what's next. **Implementation note** on the self-write filter: a path-only TTL filter is too coarse — external edits within the TTL window get swallowed. mtime-based exact-tuple matching (`(path, mtime_just_written)`) is the working design; ext4 stores nanosecond mtime so two distinct writes never collide.)
