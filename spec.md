...
**Version:** 0.75.0 (Phases 0–19.5 complete; schema version 21. Phase 20 (the 1.0 endgame) underway: v0.35.0 shipped the accessibility round-2 pass, v0.36.0 the `scripts/perf.sh` regression suite, v0.37.0 the `mdbook` documentation site (`book/`); v0.37.1 through v0.37.3 were documentation and test-suite maintenance (README + metainfo + book prose refresh, CLI parser-test consolidation, version/test-count header reconciliation). Phase 18.5's Org-mode power features shipped across v0.14.0 → v0.19.0: DEADLINE warning windows, statistics cookies, body inline checkboxes, custom TODO sequences, CLOCK time tracking, Quick Entry templates, inter-task `[[id:UUID]]` links, and time-of-day on SCHEDULED. v0.20.0 opened Phase 19.5 with an `AdwPreferencesDialog` and system-notification reminders (`reminder_at`). v0.21.0 was a behaviour-neutral maintenance pass: helper-method extraction, partial `read.rs` / `cli/main.rs` splits, and test-coverage gap fill. The six-crate workspace, single-writer SQLite worker, two-way Org vault, and Calibre-style search grammar described below are all current. Phase 19.5 productivity essentials are underway: v0.20.0 shipped system-notification reminders + the preferences dialog, v0.23.0 added subtasks (Builder Inspector "Subtasks" group, indented list nesting, Shift-drag reparent, CLI `--parent`), and v0.24.0 closed the Org property-drawer round-trip gap (custom `:KEY: value` entries survive verbatim via the new `task.extra_properties` JSON column). v0.25.0 opens Phase 19 with VTODO (RFC 5545) import + export, the CalDAV-side `.ics` bridge to Endeavour, Errands, Nextcloud Tasks, and Planify; the parser + emitter + mapper are hand-rolled stdlib, matching the Org + Todoist precedents. v0.26.0 adds the Taskwarrior `task export` JSON importer with a configurable UDA policy (`--uda-as tag|note|drop`); RFC 4122 UUIDs round-trip directly into `task.uuid`. v0.27.0 closes the Phase 19 plain-text importer arc with todo.txt support: one task per line, `(A/B/C)` priorities map to `priority-N` tags, `@context` becomes a tag, `+project` is dropped (lossy) since `--into` wins, and `due:` / `t:` key-value extensions thread to typed columns. v0.28.0 adds per-area review schedules: a new nullable `area.default_review_interval_days` column (migration 0015, schema version 15) that the Review query falls back to when a project leaves its own `review_interval_days` unset. v0.29.0 adds task dependencies (`blocked_by`): a `task_dependency` join table (migration 0016, schema version 16) plus the `is:blocked` / `is:available` predicates, a Builder Inspector "Blocked by" picker, a row pill, and `atrium-cli depend`. v0.30.0 opens the Tier 3 polish run: drag external files / URLs onto the window to capture (a window-level `gtk::DropTarget` opens Quick Entry pre-filled). v0.31.0 adds first-run onboarding (a self-clearing `AdwStatusPage` for a pristine library). v0.32.0 adds a backup / restore UI (`VACUUM INTO` snapshots under `$XDG_DATA_HOME/atrium/backups/`, keep-10, optional weekly, restore-on-next-launch via a marker file) with core helpers exposed through `atrium-cli backup`. v0.33.0 adds task templates: migration 0017's `task_template` + `task_template_item` (schema version 17), `atrium-cli task-template`, and a GUI "New from Template…" picker. v0.34.0 closes the arc: the non-Org importers move into a new `atrium-import` library crate (so the GUI can reach them) and a unified import dialog ("Import…" menu) drives all five sources through the worker with a dry-run preview. v0.38.0 adds a second kanban grouping axis: status-axis boards (§4.6) group by Org TODO-sequence keyword and dragging a card changes real task state (keyword + completion) instead of rewriting synthetic tags, plus an `atrium-cli edit --keyword` flag so the status board is fully shell-driveable. v0.39.0 → v0.40.1 are a Tier D UX pass (audit-driven): the Agenda + Forecast views merged into one entry with a Builder-only Bands/Strip layout toggle, calendar week-strip rows open their task, date-sorted lists toast on a reorder drag, a kanban Configure button, a row right-click Schedule submenu for one-pick reschedule (`win.reschedule`), and `Alt+Up`/`Alt+Down` keyboard reorder. v0.41.0 makes reminders trustworthy: a `task_reminder_fired` side table (migration 0018, schema version 18) lets the service fire overdue reminders on launch (catch-up) without re-firing, and turning notifications off no longer permanently swallows a reminder. v0.42.0 adds bulk editing (multi-select Move… / Tag… / Schedule with coalesced undo, plus multi-id `atrium-cli edit`); v0.43.0 → v0.46.0 are the kanban maturity mini-phase (richer cards, advisory per-column WIP limits, in-place per-column add, persisted intra-column order via migration 0019's `board_card_position`, schema version 19); v0.46.1 / v0.46.2 deflaked the vault-watcher integration tests (adaptive polling, then serialization). v0.47.0 opens the deferred Phase 20 tail with localisation scaffolding (§3.6): the gettext text domain, the `po/` catalogue pipeline through meson, a full marking sweep of the GTK binary's user-facing strings, and English as the first shipped catalogue. v0.48.0 → v0.66.0 are the Phase 22 de-adwaita arc, run ahead of the 1.0 tag: the C1 → C10 ladder replaced every adwaita primitive with owned plain-GTK equivalents under the owned Kanagawa Dragon stylesheet (§3.7), migration 0020 recoloured the six persisted swatches (schema version 20), libadwaita left the tree at v0.64.0 (C10), the tiling-forward layout work restored the list width cap on an owned `AtriumClamp` (v0.65.0), and v0.66.0 restored Automatic dark/light via a direct settings-portal read. v0.67.0 was a maintenance sweep (seven fixes, one restored setting, the owned symbolic icon set); v0.68.0 added the kanban keyboard path and the Quick Entry singleton; v0.69.x completed the offline Flatpak; and v0.70.0 extracted the search parser into the shared `vir-search` crate and the shared GTK widgets into `vir-gtk`, leaving a six-crate workspace. v0.71.x closed the Phase 23 codebase sweep and added the sandbox font verification; v0.72.0 repaired the last Org round-trip data losses (body lines beginning `*`, SCHEDULED warning suffixes, DEADLINE repeaters) and shipped the staged pane collapse for narrow tiles; v0.72.1 adopted vir-gtk 1.0.3 + vir-search 1.4.0 and wired the prefix/suffix/in match kinds into the evaluator; v0.72.2 is a packaging/docs maintenance stamp with no code changes; v0.72.3 is a test-infra and decision-recording stamp (the vir-gtk 1.0.4 / 1.1.0 / 1.2.0 consumer-wave adoptions ship here, a process-wide once-init removes the CI-only GTK test-init race, and the 2026-09-11 verdicts are recorded: kanban horizontal-scroll-only, EDS via `zbus` with the design deferred post-1.0, `atriumd` deferred post-1.0). v0.73.0 is the six-lens-audit lane: the reminder service fires (GLib timeout futures replace the off-runtime tokio timers that killed the loop), the worker's multi-statement sets run in transactions with `toggle_complete` honoring its documented failure policy, the kanban/backup/seed medium cluster closes, the dependency platform moves to rusqlite 0.40 + gtk4 0.11 ahead of the freeze (decision 61, riding the required vir-gtk 1.3.0), CI gains a cargo-sources.json freshness guard, and the audit's documentation sweep lands.  v0.74.0 is the consumer-wave stamp: list rows, groups, and alert dialogs come from the shared vir-gtk 1.4.0 widget kit (the duplicated builders are deleted; the async chooser and Page/Bin helpers stay app-side), and the search grammar rides vir-search 1.4.2. v0.74.1 is a release-integrity stamp: it restores the VERSION file that the v0.74.0 stamp commit emptied (meson reads the file directly, so the empty file silently mis-versioned every meson configure) and guards the sync in the ship gate, CI, and the test suite. v0.75.0 is the final-audit fix wave: the debug fixture generator rides the single-writer worker, the vault ledger seed and the import/backup blocking work leave the main thread, an `:ID:`-less vault file no longer duplicates its project, weekday names parse in CLI dates, `is:archived` / `is:queued` are recorded as reserved (§4.3.7), and the docs set is reconciled with the shipped code across schema, gtd-patterns, keymap, §3.4/§6/§9/§10, the CLI usage text, and the search-help popover. See `patchnotes.md` for the full arc.)
...
Atrium is a native Linux task manager that synthesises four traditions into one application: **Org-mode's data discipline** (UUIDs everywhere, plain-text round-trip, three repeater semantics, contexts-as-tags, full bidirectional vault mirror), **Things 3's calm** (six canonical lists, deliberate omission, the `When`/`Deadline` distinction), **OmniFocus's depth** (defer dates, sequential projects, forecast, review queues, perspectives), and **Calibre's search vocabulary** (boolean expression grammar, regex match modifiers, `is:` predicates, sort modifiers, date keywords). The synthesis isn't a clone of any one of them — it's the first Linux-native productivity app that lets a user keep all four conventions on tap from the same data store.
...
### 3.6 Localisation (Phase 20, v0.47.0)

The GTK binary is translatable via gettext. Scaffolding-only at 1.0: English is the only shipped catalogue; the machinery is what the release delivers.

- **Text domain `atrium`**, bound at startup by `atrium/src/i18n.rs::init()` (setlocale → bindtextdomain → textdomain) before any GTK type exists, so GtkBuilder templates translate at inflate time. The localedir resolves runtime `ATRIUM_LOCALEDIR` → compile-time default baked by `build.rs` (meson exports the install path) → `/usr/share/locale`.
- **Catalogue layout:** `po/` holds `POTFILES`, `LINGUAS`, the checked-in `atrium.pot`, and per-language `.po` files; meson's `i18n.gettext()` (glib preset) compiles and installs the `.mo` files. Regenerate the pot with `meson compile -C <builddir> atrium-pot` (needs xgettext ≥ 0.25 for Rust extraction).
- **What's translated:** the GTK binary's user-facing strings (via `crate::i18n`'s `gettext` / `ngettext` / `pgettext` re-exports and the `gettext_f` / `ngettext_f` named-placeholder helpers), the `.ui` files (`domain="atrium"` on `<interface>`), the GSettings schema summaries, the desktop entry, and the metainfo's store copy (release notes carry `translate="no"` — every future `<release><description>` must too).
- **What's deliberately not:** `atrium-cli` and every library crate (their output is script-parseable contract surface), the `--debug` tooling, tracing/log output, and file-format tokens (Org keywords, search-grammar operators, inline-capture syntax) — those are grammar, not prose. Placeholder sentences *containing* grammar tokens are translatable with translator comments pinning the tokens.
...
Phase 22 moved Atrium off libadwaita to plain GTK4 with an application stylesheet it owns outright (complete at v0.64.0; the C1 → C10 ladder is in roadmap Phase 22). GTK4 stays because it is Wayland-native; the GNOME design language does not. libadwaita is gone from the dependency tree; `cargo tree` shows zero adwaita. The look is **flat, calm, gently rounded, tiling-first**, Kanagawa Dragon throughout, with dragonYellow (`#c4b28a`) as the accent. The square, brutalist stance of the sibling de-adwaita apps (Hermitage / Viaduct / Conservatory) was softened for Atrium after seeing it live at C9 (v0.62.0): Atrium is a Things-3-style surface, not a utilitarian tool, so it carries rounding (controls ~8px, cards / popovers / toasts ~12px, switches and pills fully round) and a soft drop shadow on floating panels. Chrome stays flat, 1px hairline borders, no gradients. The bundled-font typography (§ Simple Mode / Builder) is unchanged by the toolkit swap.

- **The sheet is generated Rust, not a `.css` asset.** `atrium/src/ui/theme.rs` holds the Kanagawa Dragon palette as consts and splices them into a template (`%TOKEN%` replacement, no GTK custom properties). It installs display-wide at `STYLE_PROVIDER_PRIORITY_USER + 1`, just under `data/style.css` (which loads at the same priority but later, so its per-surface rules still win the ties), so a user's themed `~/.config/gtk-4.0/gtk.css` cannot half-override either. Rather than absorb `data/style.css`, the generated sheet supplies what libadwaita used to: a `@define-color` block redefining every adwaita colour name `style.css` still references (in Kanagawa hues) plus the flat-but-rounded base widgets. `style.css` keeps its per-surface tweaks and the bundled-font typography.
...
- **Atrium ships the dark Kanagawa Dragon sheet.** The `theme` apply (boot + Preferences) sets GtkSettings' `gtk-application-prefer-dark-theme` in place of `adw::StyleManager`; the `theme` GSettings key stays. **Since v0.66.0 "auto" follows the desktop** through a direct `org.freedesktop.portal.Settings` read over `gio` D-Bus (`ui::color_scheme`), which is what `adw::StyleManager`'s `Default` scheme used to do: one synchronous read before the first frame, a `SettingChanged` subscription for live switching, and a degrade to dark when no portal answers. The user's `theme` choice folds *over* the system value, so a pinned "dark" or "light" survives a system flip. With a dark-only palette a resolved "light" still only nudges the prefer-dark hint for GTK-default-drawn bits the owned sheet does not cover; a true light (Lotus) palette is post-1.0 work and is the real consumer this read exists to serve. **Decoration posture (decided 2026-07-17, for the C8 shell cut): window-control buttons are hidden.** On a tiling compositor the compositor owns close/maximize/minimize, so Atrium's headerbars carry only their functional controls (menu, search, new-task, per-dialog actions) and no close/max/min cluster; `Ctrl+Q` quits, and the compositor's binds close windows. This matches the portfolio pattern (Hermitage Phases 13/14, Viaduct v3.0.0). GNOME-Shell users still close through the shell's own affordances. No functionality may depend on chrome a user has hidden.
...
**`task_reminder_fired`** (v0.41.0) — companion to `task.reminder_at` (see the `task` table): `task_id` + `reminder_at` + `fired_at`, one row per notification actually sent. The composite match on the current `reminder_at` means moving a reminder re-arms it; the side-table shape keeps a fire from bumping `task.modified_at`.

**`board_card_position`** (v0.46.0) — persisted intra-column ordering for kanban boards: `(perspective_id, column_key, task_id, position)`, PRIMARY KEY over `(perspective_id, column_key, task_id)`, FK CASCADE on both `perspective` and `task`. `column_key` is the column value (tag name or status keyword) stored lowercased, matching the case-insensitive column match used everywhere else (the trailing "Other" bucket keys on `OTHER_COLUMN_LABEL`). Renderers read the side table to apply saved order after the tag/status projection; `reorder_board_column` (worker command + `atrium-cli perspective reorder`) renumbers integer positions per drop. It carries no Org meaning (like reminders), so it never round-trips to the vault. Advisory to the projection: a card whose tag/status no longer maps to the column simply doesn't render there; the row lingers harmlessly.
...
| `tag:*x` | suffix | matches `homework` (since vir-search 1.3.0; in-memory only) |
| `tag:(a,b)` | in-list | OR over the members (since vir-search 1.3.0; in-memory only) |
...
- **Bulk editing (v0.42.0).** Selecting two or more tasks reveals a selection bar (both modes). Beyond the v0.1.8 Complete / Delete actions it carries **Move…** (choose a project or Inbox for the whole selection), **Tag…** (add or remove one tag across the selection; already-satisfied tasks are skipped), and a **Schedule** menu (the same quick-schedule keywords as the per-row submenu). Each shows one coalesced undo toast that restores every affected task's prior project / tag set / schedule. All three reuse the single-task worker calls (`update_task`, `ensure_tag` + `set_task_tags`) and the pure `parse_quick_schedule` mapping; no schema change. CLI parity: `atrium-cli edit` accepts several leading task ids (`edit 42 7 13 --project …`).
