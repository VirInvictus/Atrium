...
**Version:** 0.10.0 (Phases 0–16 shipped; v0.9.0 extracted Org projection into its own `atrium-org` workspace crate. **Phase 17 (vault → DB sync) first slice shipped at v0.10.0** — `notify`-backed `VaultWatcher` task; `RecentWrites` self-write filter (mtime-based exact match — see *implementation note* below); reader→DB diff by `:ID:` covering CREATE / UPDATE / DELETE; `:ID:` allocation for headlines added in Emacs without one; `TaskUpdate.completed_at` setter so `CLOSED:` cookies round-trip into the DB without stamping `now()`; ergonomic `spawn_org_vault_with_watcher` builder spawning writer + watcher with one shared `RecentWrites` set. Three integration tests pin external add / external edit / external delete. Conflict detection (mtime race), malformed-file pause/resume, RRULE divergence detection on read-back, and the agenda-parity acceptance test land across the v0.10.x patch arc. **Implementation note** on the self-write filter: a path-only TTL filter is too coarse — external edits within the TTL window get swallowed. mtime-based exact-tuple matching (`(path, mtime_just_written)`) is the working design; ext4 stores nanosecond mtime so two distinct writes never collide.)
