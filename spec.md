...
**Version:** 0.10.2 (Phases 0–16 shipped; v0.9.0 extracted Org projection into its own `atrium-org` workspace crate. **Phase 17 first + next + reliability slices shipped at v0.10.0 / v0.10.1 / v0.10.2.** v0.10.0: `notify`-backed `VaultWatcher`; `RecentWrites` self-write filter (mtime-based exact match — see *implementation note* below); reader→DB diff by `:ID:`. v0.10.1: **GUI participates in the two-way loop** via `spawn_vault_loop`; **conflict detection** preserves external edits as `<file>.atrium.bak.<UTC-timestamp>` per rule 5 below; **sidecar** at `<vault>/.atrium/config.toml`; **`VaultEvent` channel** surfaces toasts; **DomainError** + `UiError` + `AtriumError` enforce subtask-stays-in-project + non-empty-filter invariants. v0.10.2: **malformed-file pause/resume** — parse failure pauses sync for that file, `ParseFailed` once on transition + `ParseRecovered` on recovery; **custom-keyword fix** — v0.10.0's watcher dropped `OrgKeyword::Custom` on create and never synced keyword changes on existing rows; new `TaskUpdate.orig_keyword` + watcher round-trips `WAITING`/`IN-PROGRESS`/etc. verbatim per rule 1 below; **file removal** — `rm` of a vault file retains tasks (DB canonical) and surfaces a `FileRemoved` toast; concurrent-edit + large-file (1K-task project) parse latency test scenarios pinned. RRULE divergence detection and the agenda-parity acceptance test land at v0.10.3 (Phase 17 closer). **Implementation note** on the self-write filter: a path-only TTL filter is too coarse — external edits within the TTL window get swallowed. mtime-based exact-tuple matching (`(path, mtime_just_written)`) is the working design; ext4 stores nanosecond mtime so two distinct writes never collide.)
