...
**Version:** 0.8.0 (Phase 15.75 complete; v0.6.x screenshot-cleanup arc complete; v0.6.19 roadmap revision retired Things 3 import + promoted Org-mode/Todoist to first-class import slots + added Phase 19.5 productivity essentials surfaced by the gap-analysis pass against Errands / Planify / Endeavour / Things 3 / OmniFocus / Taskwarrior / Todoist; **v0.7.x line:** visual-fusion + whitespace polish (v0.7.0–v0.7.1, v0.7.5), Review canonical page absorbs the Weekly Review perspective + sidebar "Lists" auto-title retired (v0.7.2), inspector check-off + perspective editor dialog (v0.7.3), task-level Mark Reviewed via migration 0006 + 7-day exclusion (v0.7.4); then the eleven-patch **Phase 16 build-out** (v0.7.6 → v0.7.18) — Phase 16 foundation + atomic-write helper + vault-path GSettings (v0.7.6); hand-rolled `atrium-core::sync::org` parser covering headlines / TODO·DONE·CANCELLED keywords / SCHEDULED·DEADLINE·CLOSED cookies / repeaters / headline tags / :PROPERTIES: drawer / body / nested subtasks (v0.7.7); matching emitter so parse → emit → re-parse is stable for every spec §7.3 construct (v0.7.8); one-shot importer `atrium-cli import org PATH` (v0.7.9); inverse vault writer `atrium-cli export org PATH` (v0.7.10); lossless JSON snapshot export `atrium-cli export json PATH` (v0.7.11); custom-keyword round-trip via migration 0007's `task.orig_keyword` (v0.7.12); file-level metadata (`#+TITLE:` + top-level `:PROPERTIES:` block) via `parse_org_text_with_meta` / `emit_org_text_with_meta` + `OrgFile` struct + `NewProject.{last_reviewed_at, archived_at}` (v0.7.13); multi-file vault walk + `WorkerHandle::ensure_area` (v0.7.14); post-write integrity check (v0.7.15); auto-debounced worker write hook `WorkerHandle::spawn_with_vault(conn, VaultConfig { root, read_pool })` + `VaultWriter` task with ~100ms debounce (v0.7.16); round-trip test fixture across five complicated `.org` files asserting source ↔ regenerated AST equality, plus `NewTask.completed_at` and CANCELLED-in-orig_keyword fixes surfaced by the suite (v0.7.17); GUI vault integration — GTK binary reads `vault-path` GSettings on boot and calls `spawn_worker_with_vault` so every DB write auto-flushes (v0.7.18). **v0.8.0** stamps Phase 16 complete and rolls the maintenance pass per release discipline — worker test split into `worker_tests.rs`, dead-code prune in the Org writer, comment audit stripping per-patch v0.7.X markers, and the four-doc sweep)
